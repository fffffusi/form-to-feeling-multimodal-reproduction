...
=========================
Full-corpus affective (Kansei) extraction with Qwen-plus.

Dimensions extracted (matches Appendix A.3 of the paper):
...
Prompt: lexicons/kansei_extraction.txt  (used as system message; enables prompt cache)
Model:  qwen-plus
Concurrency: 10

Outputs:
- data/extraction/checkpoint_kansei.csv          (one row per review, raw_json)
- data/extraction/llm_kansei_extractions.parquet (long format, one row per match)
- data/extraction/llm_kansei_extractions.csv     (long format CSV backup)
- data/extraction/llm_kansei_metadata.json
"""
...
PROMPT_FILE = PROJECT_ROOT / "lexicons" / "kansei_extraction.txt"

OUTPUT_DIR = PROJECT_ROOT / "data" / "extraction"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

CHECKPOINT_CSV = OUTPUT_DIR / "checkpoint_kansei.csv"
FINAL_PARQUET = OUTPUT_DIR / "llm_kansei_extractions.parquet"
FINAL_CSV = OUTPUT_DIR / "llm_kansei_extractions.csv"
METADATA_JSON = OUTPUT_DIR / "llm_kansei_metadata.json"
...
HTTP_TIMEOUT = httpx.Timeout(timeout=60.0, connect=10.0, read=50.0, write=10.0, pool=5.0)

# Whitelisted affective dimension labels (Chinese tokens are the keys used by the prompt).
...
def load_system_prompt():
    """Read the prompt file and return the part before the '## 待处理评论' marker as the system prompt."""
...
def get_cached_tokens(usage):
    """Best-effort extraction of the cached prompt-token count from the usage object."""
    if not usage:
        return 0
...
def parse_matches_json(raw):
    """Extract the 'matches' array from the LLM output."""
    if not raw:
        return None, "empty"
    raw_strip = raw.strip()
...
def validate_matches(matches, comment_text):
    """Filter out hallucinated spans, non-whitelisted dimensions, and abnormal polarity values."""
    if not matches:
        return [], 0
    valid = []
    n_inv = 0
    for m in matches:
        ...
        if span not in comment_text:
            n_inv += 1
            continue
        if dim not in KANSEI_DIMENSIONS:
            n_inv += 1
            continue
        if pol not in (1, -1, 0):
            n_inv += 1
...
def call_api(client, system_prompt, user_text):
    """Single API call. Returns (raw_str, usage_dict, latency, error)."""
    user_msg = f'## 待处理评论\n\n评论: "{user_text}"\n\n输出:'
    t0 = time.time()
    try:
...
def process_one(client, system_prompt, row_id, text):
    """Process one review: API call -> parse -> validate. Returns one checkpoint row dict."""
...
def main():
    ...
    if CHECKPOINT_CSV.exists():
        ...
        # Only rows without an error are treated as done; failed / empty rows are re-tried on rerun.
    ...
    if len(todo_df) > 0:
        ...
        with ThreadPoolExecutor(max_workers=CONCURRENCY) as executor:
            ...
            with tqdm(total=len(futures), desc="感性抽取", unit="条",
    ...
    # Merge product_id / commentId while preserving original order.
